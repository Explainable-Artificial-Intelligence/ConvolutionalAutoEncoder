...
class SklearnCAE(BaseEstimator, TransformerMixin):
    ...
    def _build_network_topology(self, restore=False):
        ...
        if restore:
            self.input_images = tf.get_default_graph().get_tensor_by_name("Input_Images:0")
        else:
    ...
    def _train_model(self, train_data):
        ...
        # set ANN status
        # op = self.ann_status.assign("training")
        self.tf_session.run(tf.assign(self.ann_status, b'training'))
        ...
        for epoch_i in range(start_epoch, self.n_epochs):
            ...
            if not self.tf_session.run(self.ann_status) == b'training':
                break
            # set number of trained epochs:
            self.tf_session.run(tf.assign_add(self.trained_epochs, 1))
        ...
        if self.tf_session.run(self.trained_epochs) >= self.n_epochs:
            self.tf_session.run(tf.assign(self.ann_status, b'completely trained'))
        else:
            self.tf_session.run(tf.assign(self.ann_status, b'partly trained'))
    ...
    def update_ann_status(self, status):
        ...
        if status == "stop":
            ...
            if self.tf_session._closed:
                ...
                # self._close_session()
                return
            self.tf_session.run(tf.assign(self.ann_status, b'stop', use_locking=True))
    ...
    def _close_session(self):
        ...
        """
        if self.session_saver_path is not None or not self.tf_session.run(self.ann_status) == b'training':
            self.tf_session.close()
            tf.reset_default_graph()
