...
function createANNLayerPreview(width, height, layerWidth, layerHeight, stackCount, filtersize, layerType, layerIdx, linkedLayer) {
    //create plot pane:
    if (layerType === "encoder") {
        var id = "encoder_layer_" + layerIdx;
        var title = "Encoder Layer " + layerIdx;
        var parentNode = "#encoder";
    } else if (layerType === "decoder") {
        var id = "decoder_layer_" + layerIdx;
        var title = "Decoder Layer " + layerIdx;
        var parentNode = "#decoder";
    } else if (layerType === "output") {
        var id = "output_layer";
        var title = "Output Layer ";
        var parentNode = "#output";
    } else {
        var id = "input_layer";
        var title = "Input Layer ";
        var parentNode = "#input";
    }

    if (layerType === "decoder") {
        var plot = d3.select(parentNode)
            .insert("svg", "svg")
    } else {
        var plot = d3.select(parentNode)
            .append("svg")
    ...
    if (parentNode === "#encoder" || parentNode === "#decoder") {
        // add delete button:
        var deleteBtnBorder = plot.append("rect")
            ...
            .style('fill', "none")
            .style('fill-opacity', 0.2);
        var deleteBtn = plot.append("text")
            .attr("class", "delete_button")
            .attr("x", width - 12)
            .attr("y", 12)
            .style('fill', "orange")
    ...
    // add title:
    var titleDescription = plot.append("text")
    ...
    function toggleSelection() {
        if (selected) {
            border.style("stroke-opacity", 1.0);
            selected = false;
        } else {
            border.style("stroke-opacity", 0.5);
            selected = true;
        }
    }
